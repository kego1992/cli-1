...
class ImportCommand extends Command {
  async run() {
    ...
    const oauth = flags.oauth
    let host = process.env.CS_CMA_HOST || self.cmaHost
    ...
    return new Promise(function (resolve, reject) {
      ...
      } else if (oauth && oauth !== undefined && targetStack && targetStack !== undefined) {
        parametersWithAuthToken(
          oauth,
          targetStack,
          data,
          ...
          host,
          backupdir,
          process.env.CLEAN_UP_BACKUP_DIR || false
        )
        .then(() => {
          return resolve()
        })
...
ImportCommand.examples = [
  ...
  'csdx cm:import -o <oauth> -s <stack_ApiKey> -d <path/of/export/destination/dir>',
]
ImportCommand.flags = {
  config: flags.string({
    char: 'c',
